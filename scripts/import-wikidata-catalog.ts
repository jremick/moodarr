...
function assertRecoveryDatabaseReady(dbPath: string, source: string, expectedRefreshRequired: number) {
  ...
  try {
    ...
    const aiRerankVisibilityMigration = inspection.prepare("SELECT 1 AS value FROM schema_migrations WHERE id = '033_ai_rerank_fallback_visibility'").get();
    ...
    throw new Error("Trusted catalog refresh requires a stopped database that has completed the current schema-33 migrations.");
  } finally {
    inspection?.close();
  }

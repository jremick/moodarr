...
const auditCandidateValidationWorkflow = () => {
  inspectWorkflow(VALIDATE_CANDIDATE_WORKFLOW_PATH, (workflow) => {
    ...
    const upgradeContext = `${VALIDATE_CANDIDATE_WORKFLOW_PATH}.jobs.upgrade-rollback`;
    const upgrade = workflowJob(workflow, "upgrade-rollback", VALIDATE_CANDIDATE_WORKFLOW_PATH);
    const beta2Upgrade = namedStep(upgrade, "Validate direct beta.2 upgrade and cold rollback", upgradeContext);
    const beta2Run = expectRunContains(beta2Upgrade, [
      "set -euo pipefail",
      "npm run --silent validate:beta2-upgrade",
      ...
      '> "$REPORT_PATH"'
    ], `${upgradeContext} direct beta.2 validation`);
    expect(!/--allow-(?:local-image|dirty|emulation)|\|\| true/.test(beta2Run), `${upgradeContext} beta.2 validation must remain release eligible and fail closed`);
    const beta2Environment = mappingField(beta2Upgrade, "env", `${upgradeContext} beta.2 validation`);
    expectEqual(beta2Environment.CANDIDATE_IMAGE, "${{ steps.candidate.outputs.image }}", `${upgradeContext} beta.2 candidate digest`);

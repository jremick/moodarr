...
```bash
docker pull ghcr.io/jremick/moodarr:v0.1.0-alpha.15
docker run --rm -p 4401:4401 \
  -v moodarr-data:/data \
  -e MOODARR_ADMIN_TOKEN="replace-with-a-long-random-token" \
  -e MOODARR_ADMIN_AUTO_SESSION=true \
  ghcr.io/jremick/moodarr:v0.1.0-alpha.15
```
...
The template at `unraid/moodarr.xml` targets the immutable alpha image tag `ghcr.io/jremick/moodarr:v0.1.0-alpha.15`. For local-only testing, build and tag a local image as `moodarr:local` and adjust the template repository field.

...
Backups contain Plex, Seerr, and signed-in-user Plex credentials. They can also contain an inert OpenAI key when the volume was previously used by a source build. Encrypt them, restrict access, and never attach them to public issues or support requests. File mode `600` is useful defense in depth, but it is not encryption and does not protect a copied disk, exported share, or misplaced archive.

...
| `exactOriginSession` | Before unlock, privately compare `MOODARR_WEB_ORIGIN`, the browser address-bar origin, and Docker Manager's WebUI shortcut target. Direct Unraid mode makes all three the same IP origin; custom hostname or HTTPS mode opens that configured origin directly and does not use the IP shortcut. Admin token exchange creates the expected host-scoped session there, a cookie-authenticated non-mutating POST succeeds from that origin, and the alternate IP or hostname origin is rejected. |

...
Beta.2 also updates the default engine to `moodrank-v0.5.3`, generated features to `moodrank-v0.4-features-v5`, and fingerprint rules to `fingerprint-rules-v4`. Follow the [full feature/fingerprint refresh procedure](MOODRANK_CORRECTNESS_UPGRADE.md) on an authorized offline copy before upgrading a large catalog, including an earlier beta.2 source/EXP installation. Startup backfill alone does not establish complete coverage.

Source checkouts use `npm run backfill:features:bulk`. Container installations use the compiled entry point in the exact new image, as shown below. The `--defer-feature-fts` option rebuilds feature FTS after the batches; it does not skip fingerprint generation. Verify zero stale feature/fingerprint rows, complete FTS coverage, and preserved imported mood sources, feedback, and historical profiles before returning the upgraded data to service. The skip-fingerprint repair command is not sufficient for this upgrade. A rollback must restore the prior derived data with its matching code. Provider embeddings are not regenerated by this procedure, and the new source-only ranking/discovery experiments remain disabled.

For a container installation, first stop Moodarr, make a complete cold backup, and restore it into a separate rehearsal data directory owned by UID/GID `999:999`. Pull and verify the exact new image before disconnecting the refresh from the network. This example refuses a missing directory or a directory attached to a running container. Replace both placeholders with the verified image digest and the absolute path of the restored copy; never run it against a database used by a running server.

```bash
set -eu
candidate="ghcr.io/jremick/moodarr@sha256:<validated-candidate-digest>"
refresh_data="/absolute/path/to/restored-moodarr-data"
test -d "$refresh_data"
test -f "$refresh_data/moodarr.sqlite"
test -z "$(docker ps --quiet --filter volume="$refresh_data")"
...
docker run --rm --network none --read-only --init \
  ...
  --mount type=bind,src="$refresh_data",dst=/data \
  --env MOODARR_REQUIRE_ADMIN_TOKEN=true --env MOODARR_PLEX_AUTH_ENABLED=false \
  "$candidate" dist/server/backfillContentFingerprints.js \
  --refresh-features --defer-feature-fts
```

The one-shot command starts no HTTP server or scheduler and has no network access. Its Plex sign-in override applies only to this process and does not change saved settings. A successful exit must include the final refresh summary and a complete FTS rebuild. Follow the linked correctness procedure for database and retained-state checks, then rehearse the new server on that copy. Before the final update, stop the live server and take a fresh cold backup so that writes since the rehearsal are preserved. Repeat the refresh and checks on the final data while the server remains stopped. For a named Docker volume, use its existing name in the running-container check and `type=volume` mount, and confirm it exists with `docker volume inspect` before running.

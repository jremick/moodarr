...
2. **Validate the published candidate.** Independently pull the candidate by digest and complete every candidate-validation row, including clean Docker, Compose, and Unraid paths plus raw digest, image identity, SBOM, provenance, GitHub attestation, and exact-digest image-scan evidence. A failed candidate is abandoned; the workflow refuses to overwrite its tag and requires a new source commit.

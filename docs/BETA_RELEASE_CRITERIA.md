...
4. **Create the protected Git tag and approve public announcement.** Only after the approved image-promotion workflow succeeds, manually create the protected semantic Git tag at the exact candidate commit. Verify the peeled Git commit, both image-tag digests, and the attestation once more. Prepare the GitHub prerelease as a draft with the verified digest, beta support boundary, known limitations, and exact catalog asset; read the asset back and revalidate its bytes before publishing the immutable prerelease.
...
GHCR tag creation is not an atomic create-only operation. The workflow creates the version tag only after a registry `404`; an existing `200` response is read-only and accepted only for the exact approved manifest, so an interrupted post-write verification can be resumed safely through another `beta-release` approval. It then re-reads both candidate and version manifests. A separately authorized package writer could still race the create request, so keep package-write access restricted, do not push GHCR release tags outside the workflow, and treat any unexpected media type, bytes, or digest as a failed promotion requiring investigation. The protected Git tag is a separate manual post-promotion action and must never be created before the Tier 3-approved image workflow succeeds.

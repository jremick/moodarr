...
- The exact published digest passes every automated native Linux `amd64` candidate job: clean Docker/Compose install, alpha.21/beta.1/beta.2 upgrades, restart, cold-backup rollback and supply-chain verification. Source or EXP runs cannot replace this evidence.
...
The original comprehensive gate and independent evaluation protocol still define completion. This release-specific decision does not approve beta.4 or later releases, stable releases, disabled experiments, provider activation, native-client installation or an EXP deployment.

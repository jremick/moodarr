...
### Controlled independent semantic-discovery arm

An optional source-only evaluation seam can inject a validated offline exact semantic index into `RecommendationEngine` or retrieval options. It discovers query/example neighbours before the existing candidate cap, independently of the lexical/mood/popularity window, with bounded source fusion, current input-hash and eligibility rechecks, cancellation/fallback and distinct provenance. Normal application callers do not inject it, so it is disabled by default. The active experimental arm is identified by the engine suffix `+local-semantic-discovery-v1`; model resources and independent relevance evidence are not supplied or implied. See [independent retrieval experiment](MOODRANK_INDEPENDENT_RETRIEVAL_EXPERIMENT.md) for activation boundaries, containment limits and remaining work.

### Controlled shared-intent and ranking ablations

An optional tenth engine constructor argument selects five independent source-only experiments: shared signed viewing intent, normalised example feedback, a total learned-personalisation budget, experiential diversity and contribution-based explanations. No ordinary caller enables them. They are separately versioned by `+intent-ranking-v1-<mask>` and preserve original hard-filter authority and optional/off-by-default trace persistence. Initial visible-fixture ablations include regressions, so promotion is not approved. See [experiment contract](MOODRANK_INTENT_RANKING_EXPERIMENT.md) for exact semantics, controls, evidence limitations and the reproducible seven-arm diagnostic runner.

### v0.5.3 follow-through and review candidate

Direct description-backed prohibitions now survive final scoring; content cues correctly recognise bounded non-/free and comparative negation. Features advance to v5 and fingerprint rules to v4. Shared intent v2 preserves affirmative surface/context, does not invent coping goals, handles denied first-person states, and distinguishes degree-derived genre preferences from strict/UI exclusions. Its mood fallback uses the same signed projection. The independent review candidate (source-only mask 59) uses audit-only total personalisation measurement instead of the regressing fixed-eight cap; existing callers activate neither ranking nor semantic experiments. A real loopback-only, pinned-model Ollama adapter and an explicit cold-catalogue index-preparation command are available for controlled operator evaluation. The independent evaluator supports a named review arm and an optional strictly hash-bound precomputed semantic bundle without calling that adapter or weakening its read-only/no-network policy. See [engineering follow-through](MOODRANK_COMPLETION_2026_09.md).

# Exact-digest beta.3 catalog validation

This procedure supplies the mandatory catalog row in the [beta.3 fixes profile](BETA_RELEASE_CRITERIA.md#approved-beta3-fixes-release-profile). Run it from the clean, frozen candidate checkout, against its published immutable digest, after the existing [candidate identity and supply-chain gates](RELEASE.md#original-comprehensive-two-stage-beta-promotion) pass. An unpublished image, earlier source benchmark or another digest cannot pass this row.
...
docker_catalog image inspect "$CAT_IMAGE" | jq -e --arg id "$CAT_IMAGE_ID" --arg r "$CAT_REVISION" --arg image "$CAT_IMAGE" '.[0] |
  ...
  .Config.Labels["org.opencontainers.image.revision"]==$r and .Config.Labels["org.opencontainers.image.version"]=="0.1.0-beta.3" and
...
jq -n --arg revision "$CAT_REVISION" --arg digest "$CAT_DIGEST" --arg imageId "$CAT_IMAGE_ID" --arg assetSha256 "$CAT_ASSET_HASH" \
  '{version:"0.1.0-beta.3",revision:$revision,digest:$digest,imageId:$imageId,assetSha256:$assetSha256,assetRecords:90397}' > "$CAT_DIR/public/manifest.json"
...
| --- | --- |
| `schema`, `status`, `observedAt`, `reviewedBy` | `moodarr-beta3-catalog-validation-v1`; `Pending`, `Passed` or `Failed`; UTC time; reviewer and review time |

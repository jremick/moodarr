...
- `--mode incremental`: default. Use for daily/lightweight RecentChanges-derived changed-QID files. Missing source IDs are ignored because the file is not a complete source snapshot. Ordinary incremental mode rejects `--expected-file-sha256` because it does not provide whole-snapshot replacement semantics.
...
- `--mode incremental --rehydrate-required`: reserved for the stopped alpha.21-to-beta.1 Wikidata-only trusted-recovery procedure in [Upgrading](UPGRADING.md). It rejects `--limit` and any non-Wikidata source, binds the complete asset's expected source count and SHA-256, discovers latent type repairs and the full affected-source closure in a read-only pass, then requires the exact refresh-source count, repair count, recovery count, and canonical recovery-plan SHA for the atomic writing pass.

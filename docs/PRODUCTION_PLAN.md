...
The sequenced work packages, current dispositions, dependencies, and acceptance checks are maintained in the [roadmap](ROADMAP.md). The list below summarizes the longer-term scope.

- Complete actual desktop/mobile Plex app checks. Earlier movie and series destination checks passed in Plex Web; native launch and missing-client behavior remain open. See [Plex and TV request validation](PLEX_AND_TV_REQUEST_VALIDATION_2026_09.md).
...
- Extend Admin job visibility only for demonstrated gaps; recent Plex and Seerr sync history already exists.
- Preserve multi-season TV input, preview invalidation, and one-write retry behavior through the [browser regression gates](../tests/browser/README.md); see [the validation record](PLEX_AND_TV_REQUEST_VALIDATION_2026_09.md).

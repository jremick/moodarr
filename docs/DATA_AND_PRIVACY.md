...
The pinned asset contains 90,397 records. Moodarr marks 82,865 of them as locally eligible for a request attempt—70,841 movies and 12,024 TV series—because they contain the required approved metadata and an unambiguous interoperability identity. Thirty-six groups share a strong importer identifier across 72 source records. Fifty-nine of those records—10 movies and 49 TV series—otherwise meet attempt requirements. Their ambiguous catalog materializations remain imported and indexed for provenance and diagnostics, but cannot independently surface in Finder or authorize request preview or creation because choosing one mapping would be ambiguous. An independently identified available Plex item may remain Finder-visible if linked later; catalog ambiguity still blocks every request action and never supplies a trusted request identifier. Eligibility is not a Seerr availability check. Eligible rows remain `unavailable`, generic search and verified-requestable-only filters exclude them, and an explicit request-attempt result is labeled **Seerr request attempt** and **Availability not checked**. Preview performs no upstream write; only a separate confirmation sends the identifier, media type, and selected TV seasons to the operator-configured Seerr service.
...
Confirmed-request responses are also treated as untrusted upstream input. Moodarr returns and stores only an allowlisted Seerr request ID and normalized status alongside the local media type, media ID, and selected seasons. It discards nested upstream users, tokens, descriptive objects, and unknown fields. The schema-29 boundary step within beta.1's final schema 31 applies the same projection to every legacy created idempotency response and clears malformed or non-created response bodies.

For an alpha.21 upgrade, affected rows are converted to generic `operational` placeholders and excluded from discovery. A Plex-only materialization can be restored only by a new Plex sync, and an affected Wikidata catalog record can be restored only from the approved catalog file for its recorded `wikidata` source through the packaged networkless importer. Moodarr preserves the prior source identity and content hashes while the materialization is marked stale; a same-payload refresh retains that content version, while an authoritative changed payload records new hashes and a new content version. A read-only importer pass reports the complete asset count, expected source-specific pending count, latent movie/TV repairs, recovery closure, and a canonical SHA-256 of the pre-write plan without changing the database. The stopped writing pass must bind that exact evidence and rolls back every source, identity, metadata, cache, and derived-index change if any binding or final check differs. A non-QID external identity corroborated by Plex, Seerr, request, or request-operation state is never removed by type repair. User, recommendation-history, review, and trace rows are also preserved. Admin diagnostics exposes aggregate pending counts. Seerr-only rows without a trusted descriptive source remain operational placeholders; request state and factual interoperability IDs remain available, but titles, summaries, artwork, features, embeddings, and search replicas are not reconstructed from Seerr/TMDB content.

Schema 31 also retains only aggregate per-item quarantine state when an upstream integration record supplies multiple strong identifiers that resolve to different Moodarr items. Moodarr skips that conflicting record without rebinding stored IDs, continues safe sibling records, and blocks requests for the quarantined item. The quarantine stores a fixed reason, timestamps, and an occurrence count rather than the upstream payload, and is separate from catalog importer ambiguity. After operators correct upstream identities, one successful Admin full sync with both Plex and Seerr enabled atomically clears only quarantines not seen since that run began and rebuilds their catalog search entries. Reproduced conflicts remain blocked; sync status and support output report only the aggregate cleared count.
...
- Request audit history and user identity rows do not currently have a complete self-service retention/deletion policy. Successful request-operation markers and their tightly allowlisted response (`id` and normalized status) are retained so an expired cleanup cannot make an old confirmation generation valid again; failed operation rows expire after 90 days. Treat the absence of configurable request/audit retention as a beta limitation.

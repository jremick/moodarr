...
describe("MoodRank model-selection contract", () => {
  ...
  it("rejects a challenger when paired case uncertainty breaches the precommitted reference margin", () => {
    ...
    expect(evaluated.productionPromotionEligible).toBe(false);
    expect(evaluated.productionRejectionReasons).toContain("minimum_reference_quality_delta_lower_95_not_met_or_missing");
  });
  ...
  it("keeps diagnostic quality references comparable but never promotes diagnostic challengers", () => {
    ...
    expect(challenger.productionRejectionReasons).toEqual(expect.arrayContaining([
  ...
  it.each([
    ...
    const result = evaluateModelSelection(manifest(), reports);
    const challenger = result.configurations.find((entry) => entry.id === "luna-medium-fast")!;
    expect(challenger.productionPromotionEligible).toBe(false);
    expect(challenger.productionRejectionReasons).toContain(expectedReason);
    expect(result.recommendedConfigurationId).toBeNull();
  });
  ...
  it("cannot recommend a challenger when the incumbent evidence is ineligible", () => {
    ...
    expect(challenger.productionPromotionEligible).toBe(false);
    expect(challenger.productionRejectionReasons).toContain("incumbent_not_production_eligible");
    expect(result.productionPromotionCandidateIds).toEqual([]);
    expect(result.recommendedConfigurationId).toBeNull();
    expect(result.decisionReasons).toContain("incumbent_not_production_eligible");
  });
  ...
  it("marks dominated configurations and lets selection policy remain a manifest-only change", () => {
    ...
    const qualityOrdered = evaluateModelSelection(matrix, expandedReports);
    const costFirst = evaluateModelSelection({
      ...matrix,
    ...
    expect(qualityOrdered.recommendedConfigurationId).toBe("second-challenger");
  ...
  it("does not turn screening evidence into a production decision", () => {
    ...
    const result = evaluateModelSelection(screening, validReports());
    expect(result.productionPromotionCandidateIds).toEqual([]);
    expect(result.recommendedConfigurationId).toBeNull();

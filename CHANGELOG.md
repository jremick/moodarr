...
- Bound every candidate-validation job to a revision reachable from current `origin/main` and to the exact GitHub artifact-attestation policy, and added credential-free anonymous exact-digest pull proof, published-index digest recomputation, exact-byte-verified Buildx installation, pinned BuildKit/SBOM tooling, attached SPDX/provenance validation, and an exact-digest Trivy evidence artifact.

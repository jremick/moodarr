...
- Hardened the provisional source/EXP embedding path so incompatible cached vectors are detected, excluded from retrieval, and gradually replaced without loading the entire catalog into memory; the official beta.1 image remains provider-locked.
...
- Bound every candidate-validation job to a revision reachable from current `origin/main` and to the exact GitHub artifact-attestation policy, and added published-index digest recomputation, exact-byte-verified Buildx installation, pinned BuildKit/SBOM tooling, attached SPDX/provenance validation, and an exact-digest Trivy evidence artifact.
...
- Bound pull-request and main CI to the exact event-source commit, added a read-only exact-source container scan with a 30-day allowlisted identity/Trivy artifact, and kept package publication permissions out of the CI path.
- Unified candidate-versus-promotion classification before privileged publication, rejected alternate tag-ref spellings that could select the weaker environment, and disabled administrator bypass on both GitHub publication environments so semantic promotion always reaches the Tier 3 reviewer gate.
- Required full catalog snapshots to pass a read-only whole-file count against an operator-supplied validated manifest before batch writes, and to match again before any unseen source records can be marked inactive, preventing empty, truncated, duplicate-heavy, or all-invalid inputs from partially updating or tombstoning a working catalog.

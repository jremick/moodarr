...
- Baked a non-overridable provider policy into the official beta.1 server bundle and OCI identity, removed provider controls from public packaging, rejected hostile environment/persisted/Admin attempts and embedding warmup, and required requested-AI searches to remain local with no OpenAI endpoint in the release artifact.

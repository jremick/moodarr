...
- Added schema 29 legacy-content sanitation for ambiguous Seerr-linked descriptions, artwork/cache rows, features, indexes, fingerprints, embeddings, and review snapshots while retaining factual IDs, user/profile/request state, and safe local relationships; operational-only Seerr rows are excluded from discovery.

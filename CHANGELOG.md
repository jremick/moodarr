...
- Removed the redundant **Available in Plex** card pill so the existing **Open Plex** action is the single availability signal, while meaningful Seerr request-state pills remain visible; legacy available rows without an item-specific link now fall back to Plex home instead of leaving an actionless card.
- Made candidate publication and supply-chain validation buffer and retry pinned BuildKit readiness checks, preventing early-output pipe failures from stranding an otherwise publishable immutable candidate.
...
- Requested a no-additional-swap container envelope by setting memory-plus-swap equal to the 2 GiB memory limit in Docker, Compose, and Unraid examples; release evidence must still prove that the Docker host enforces the swap limit or has zero usable host swap.
- Made the clean-install, upgrade/rollback, and responsiveness validators recognize Docker's observed `MemorySwap=-1` normalization only when a bounded, fail-closed read of host `SwapTotal` is exactly zero; the fallback rejects `0`, other limits, missing or malformed host evidence, and positive host swap, while responsiveness additionally rejects before/after swap-state drift. The requested 2 GiB memory-plus-swap ceiling remains unchanged.
...
- Repaired legacy alpha catalog source identities that were bound across movie/TV namespaces when both types shared a numeric TMDB ID. The stopped networkless recovery now uses a read-only discovery followed by one atomic write bound to the exact asset/counts and a canonical SHA-256 plan covering pre-write bindings, repair targets, companion rematerialization, external-ID cleanup, and final closure.
...
- Revoked the abandoned restrictive-umask candidate after testing exposed bind-mounted fixtures that were unreadable to unprivileged validation containers; the replacement harness explicitly normalizes their completed modes before read-only container use.
